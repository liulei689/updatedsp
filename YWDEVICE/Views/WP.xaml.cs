...
namespace YWDEVICE.Views
{
    ...
    public partial class WP : UserControl, IDisposable
    {
        ...
        private void serialPort1_DataReceived(object sender, SerialDataReceivedEventArgs e)
        {
            ...
            if (buffer.Length == 5)
            {
                if ((byte)buffer[1] == 0x47)
                {
                    if (CRCBuffer(buffer) == (byte)buffer[3])
                    {
                        byte[] data2 = new byte[57] { 0x03, 0x00, 0x03, 0x21, 0x32, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0x00, 0x00, 0xE0, 0x00, 0x0D };
                        sendData(new byte[5] { 0x03, 0x00, 0x03, 0x21, 0x32 }, 5);
                        Thread.Sleep(1000);
                        sendData(new byte[51] { 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0xE0, 0x00, 0x00, 0x00, 0x00, 0xE0, 0xD3 }, 51);
                        Thread.Sleep(1000);
                        //data2[55] = CRCBuffer(data2);
                        sendData(new byte[1] { 0x0D }, 1);
                    }
                }
            }
            if (buffer.Length == 4)
            {
                if (CRCBuffer(buffer) == (byte)buffer[2])
                {
                    //crc校验通过
                    byte[] data = new byte[4];
                    data[0] = 0x03;
                    data[1] = buffer[1];
                    data[2] = CRCBuffer(data);
                    data[3] = 0x0D;
                    //握手回应
                    sendData(data, 4);
